...
if len(sys.argv)<4:
    sys.exit("python baseline.py kernels dataset radius outfile lambda")
...
k = ODDSTincGraphKernel(r=radius, l=lmbd, normalization=True, version=1, ntype=0, nsplit=0, kernels=kernels, buckets=False)
...
#            print idx, ": positive semi-definite, saving..."
    output = outfile+key+".d"+dataset+".r"+str(radius)+".l"+str(lmbd)+".svmlight"
