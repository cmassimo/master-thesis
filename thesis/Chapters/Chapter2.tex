...
Unless otherwise noted, most of the material in this section has been derived
...
$\mathcal{X'_x}$ of some space $\mathcal{X'}$.
...
Haussler was able to demonstrate the positive semi-definiteness of any $K$ kernel
...
$M_{x,y} \subseteq \mathcal{X'_x} \times \mathcal{X'_y}$ and a \emph{base kernel} $k$ on it
that will hence consider only a subset of the entire cross product $\mathcal{X'_x} \times \mathcal{X'_y}$.
...
An example of feature extraction is given in Figure ??.
...
\begin{itemize}
    \item the addition of contextual information \cite{context}
