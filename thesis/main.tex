...
\thesistitle{Analysis of Graph Kernel Learning Approaches Combination} % Your thesis title, this is used in the title and abstract, print it elsewhere with \ttitle
...
Graph mining and machine learning method that use graph representation have seen
a steady growth in success and proposed approaches during the last decade.
A number of improvements over the existing techniques, have recently been
proposed, further increasing the performance limits.
These works typically addresses a specific weak point in a particular approach,
so a natural question one may want to answer is if a combination of these
improvements can outperform the single one.

In this study we analyse the combination of these improvements, namely
kernel methods with contextual informations and $EasyMKL$, a new $MKL$ paradigm
implementation, and see that in most cases the increased complexity of the
feature space derived from these combinations translates into increased
performances while maintaining good computational bounds.

The results are derived from a number of real world problems using publicly
available bio-chemical datasets.
...
\textbf{STC} & \textbf{S}ub \textbf{T}ree with \textbf{C}ontext\\
...
managed to be an invaluable presence and motivation.} 
